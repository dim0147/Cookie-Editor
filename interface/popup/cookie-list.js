(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Duplicate
        document.getElementById('convert-cookie-to-json').addEventListener('click', () => {
            if (disableButtons) {
                return;
            }

            setPageTitle('Cookie Editor - Convert Cookie to Json');
            ...
            document.getElementById('button-bar-default').classList.remove('active');
            document.getElementById('button-bar-convert-to-json').classList.add('active');
            return false;
        });
        ...
        document.getElementById('return-list-convert-to-json').addEventListener('click', () => {
            showCookiesForTab();
        });
        ...
        document.getElementById('save-import-cookie').addEventListener('click', e => {
            ...
            let cookies;
            try {
                cookies = JSON.parse(json.trim().replace(/\s/g, ''));
            } catch (error) {
        ...
        document.getElementById('convert-to-json').addEventListener('click', e => {
            
            function createImportCookie(){
                var cookie = document.querySelector('textarea').value,
                  domain = prompt("Enter Site", "www.facebook.com"),
                  output = {},
                  arrayElement = [];
                cookie.split(/\s*;\s*/).forEach(function (n) {
                  n = n.split(/\s*=\s*/);
                  const e = `{\n     "domain": "${domain}",\n      "hostOnly": true,\n      "httpOnly": false,\n      "name": "${
                    n[0]
                    ...
                    .splice(1)
                    .join("=")}"\n  }`;
                  arrayElement.push(e), (output[n[0]] = n.splice(1).join("="));
                });
                var json = JSON.stringify(output, null, 4);
                return `[${arrayElement.join(",")}]`;
            }

            let buttonIcon = document.getElementById('convert-to-json').querySelector('use');
            document.querySelector('textarea').value = createImportCookie();
            sendNotification('Json data was created');
            return;
        });
